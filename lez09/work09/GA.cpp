...
int main(int argc, char* argv[]) { 
    ...
    if (argc != 3) {
        cout << "Usage: " << argv[0] << " <filename> <configuration:CIRCLE/SQUARE>" << endl;
        return 1;
    }
    string filename = argv[1];
    string configuration = argv[2]; //CIRCLE or SQUARE
    const vector<City> cities = readCitiesFromFile(filename);
    ...
    /*
    vector<int> correct_circular_path = {1, 26, 3, 20, 28, 31, 9, 30, 8, 17, 19, 24, 10, 7, 29, 22, 32, 23, 14, 4 ,12, 21, 2, 15, 16, 18, 13, 33, 27, 34, 6, 25, 11, 5};
    cout << Loss(correct_circular_path, cities, 1) << endl; 
    */
    ...
    //////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////
    // initialize vectors to be printed
    ...
    for (unsigned int gen = 1; gen<=number_of_generations; gen++ ) {
        ...
        cout << "Shortest route found = "<< bestPath[gen-1] << endl;
    }
    ...
    string output_filename = "bestpath"+configuration+"loss"+to_string(which_loss)+".dat";
    ...
    //////////////////////////////////////////////////////////////
    // print population
    string outputLOSS = "decrease" + configuration + to_string(number_of_cities) + "loss" + to_string(which_loss) + ".dat";
    ofstream OUT;
...
double Loss(const vector<int>& permutation, const vector<City>& cities, const unsigned int which_loss) {
    ...
    for (int i = 0; i < N - 1; ++i) {
        ...
        const City& city1 = cities[city1_id-1];       //...and the cities 'book' is parsed...
        const City& city2 = cities[city2_id-1];
    ...
    const City& first_city = cities[first_city_id-1];
    const City& last_city = cities[last_city_id-1];
